...
const App: React.FC = () => {
  ...
  useEffect(() => {
    const initApp = async () => {
      ...
      // Check URL for Sync Key (Enable easier pairing)
      ...
      if (currentSettings.syncKey) {
        ...
        if (cloudData) {
          // Merge cloud data but keep the current syncKey
  ...
  const renderTool = () => {
    switch (activeTool) {
      ...
      case ToolType.AGE_CALC: return <AgeCalculator syncKey={settings.syncKey} />;
  ...
  return (
    <div className="min-h-screen flex flex-col md:flex-row bg-slate-50 text-slate-900 overflow-hidden">
      {showAdminLogin && !isAdminAuth && <AdminLogin onLogin={(id, pass) => {
        if (id === settings.adminId && pass === settings.adminPassword) {
          setIsAdminAuth(true); setShowAdminLogin(false); setActiveTool(ToolType.ADMIN); setLoginError('');
          window.history.replaceState({}, document.title, window.location.pathname);
        } else setLoginError('Invalid Admin ID or Password');
      }} onCancel={() => { setShowAdminLogin(false); window.history.replaceState({}, document.title, window.location.pathname); }} error={loginError} />}
      ...
      <aside className={`fixed inset-0 z-40 bg-white border-r w-72 transform transition-transform duration-300 md:relative md:translate-x-0 ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'} flex flex-col`}>
        <div className="flex flex-col h-full">
          <div className="p-6 pb-2 hidden md:flex items-center gap-3 mb-6 px-2">
          ...
          <div className="flex-1 overflow-y-auto px-4 md:px-6 pb-2 custom-scrollbar">
            <nav className="space-y-1">
              {getOrderedNavItems().filter(item => settings.enabledTools[item.id] !== false).map((item) => (
                <button key={item.id} onClick={() => { setActiveTool(item.id); setIsSidebarOpen(false); }} className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-200 group ${activeTool === item.id ? 'bg-indigo-600 text-white shadow-md shadow-indigo-100' : 'text-slate-500 hover:bg-slate-100 hover:text-slate-900'}`}>
                  <span className={activeTool === item.id ? 'text-white' : 'text-slate-400 group-hover:text-indigo-600'}>{item.id === ToolType.ADMIN ? <Settings size={20} /> : item.icon}</span>
          ...
          <div className="p-6 pt-0 space-y-4 flex-shrink-0 bg-white">
            {isAdminAuth && <button onClick={() => { setIsAdminAuth(false); setActiveTool(ToolType.BASIC_CALC); }} className="w-full flex items-center justify-center gap-2 px-4 py-3 bg-red-50 text-red-600 rounded-xl font-bold text-xs uppercase tracking-widest hover:bg-red-100 transition-all border border-red-100"><LogOut size={16} /> Exit Admin</button>}
          </div>
        </div>
      </aside>
        ...
        <div className="max-w-[1920px] mx-auto w-full animate-in fade-in slide-in-from-bottom-4 duration-500 flex-1">
          <div className="mb-4 sm:mb-6 ml-2 flex justify-between items-end">
            <div>
              ...
              <p className="text-slate-500 mt-1 font-medium text-xs sm:text-sm">{navItems.find(i => i.id === activeTool)?.description || 'Advanced Application Node'}</p>
            </div>
            {settings.syncKey && (
               <div className="hidden sm:flex items-center gap-2 px-3 py-1.5 bg-white border border-slate-200 rounded-xl shadow-sm mb-1">
                 ...
                 <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Supabase Connected</span>
               </div>
            )}
          </div>
