...
import { 
  Calculator, User, IndianRupee, Type, Calendar, Percent, Receipt, Ruler, CaseUpper, Menu, X, ChevronRight, QrCode, Link2, Mail, Phone, Settings, TrendingUp, FileText, PlusMinus, Binary, CalendarClock, MessageCircle, Sparkles, Languages, Search, MapPin, LogOut, Cloud, Database
} from 'lucide-react';
...
const defaultSettings = {
  ...
  enabledTools: {
    ...
    [ToolType.HISTORY]: true,
    [ToolType.ADMIN]: true,
  },
  toolOrder: [
    ToolType.BASIC_CALC, ToolType.SCIENTIFIC_CALC, ToolType.EMI_CALC, ToolType.FINANCIAL_CALC, ToolType.AGE_CALC, ToolType.HISTORY,
    ToolType.CURRENCY_COUNTER, ToolType.NUMBER_WORDS, ToolType.DAY_COUNTER, ToolType.DATE_OFFSET_CALC, ToolType.PERCENT_FINDER,
    ToolType.GST_CALC, ToolType.HSN_FINDER, ToolType.MEASUREMENT, ToolType.TEXT_CONVERTER, ToolType.FANCY_TEXT, ToolType.INDIC_KEYBOARD,
    ToolType.QR_GENERATOR, ToolType.LINK_SHORTENER, ToolType.WHATSAPP_LINK, ToolType.SEO_META_GEN, ToolType.FAKE_ADDRESS_GEN, ToolType.ADMIN
  ],
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    const initApp = async () => {
      ...
      if (currentSettings.syncKey) {
        ...
        if (cloudData) currentSettings = { ...currentSettings, ...cloudData };
        setIsCloudSyncing(false);
      }
      setSettings(currentSettings);
      const params = new URLSearchParams(window.location.search);
      if (params.has('admin') || params.get('mode') === 'admin') setShowAdminLogin(true);
      setLoaded(true);
    };
    initApp();
  }, []);
  ...
  const navItems: NavItem[] = [
    ...
    { id: ToolType.HISTORY, icon: <Database size={20} />, description: 'Records & AI Insights' },
  ...
  const getOrderedNavItems = () => {
    ...
    return settings.toolOrder
      ...
      .filter((item: NavItem | undefined): item is NavItem => item !== undefined && (item.id !== ToolType.ADMIN || isAdminAuth));
  };
  ...
  const renderTool = () => {
    switch (activeTool) {
      ...
      case ToolType.HISTORY: return <HistoryPanel />;
  ...
  if (!loaded) return <div className="min-h-screen bg-slate-50 flex items-center justify-center"><div className="w-12 h-12 border-4 border-indigo-600 border-t-transparent rounded-full animate-spin"></div></div>;

  return (
    <div className="min-h-screen flex flex-col md:flex-row bg-slate-50 text-slate-900 overflow-hidden">
      {showAdminLogin && !isAdminAuth && <AdminLogin onLogin={handleAdminLogin} onCancel={() => { setShowAdminLogin(false); window.history.replaceState({}, document.title, window.location.pathname); }} error={loginError} />}
      <header className="md:hidden flex items-center justify-between p-4 bg-white border-b sticky top-0 z-50">
        <div className="flex items-center gap-2"><div className="w-8 h-8 bg-indigo-600 rounded-lg flex items-center justify-center text-white font-bold">{settings.appName.charAt(0)}</div><span className="font-bold text-xl tracking-tight">{settings.appName}</span></div>
        <button onClick={() => setIsSidebarOpen(!isSidebarOpen)} className="p-2 text-slate-600 hover:bg-slate-50 rounded-lg transition-colors">{isSidebarOpen ? <X size={24} /> : <Menu size={24} />}</button>
      </header>
      <aside className={`fixed inset-0 z-40 bg-white border-r w-72 transform transition-transform duration-300 md:relative md:translate-x-0 ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'} flex flex-col`}>
        <div className="flex flex-col h-full">
          <div className="p-6 pb-2 hidden md:flex items-center gap-3 mb-6 px-2"><div className="w-10 h-10 bg-indigo-600 rounded-xl flex items-center justify-center text-white font-bold shadow-lg shadow-indigo-200">{settings.appName.charAt(0)}</div><div className="flex flex-col"><div className="flex items-center gap-2"><span className="font-bold text-lg leading-tight truncate max-w-[120px]">{settings.appName}</span>{settings.syncKey && <Cloud size={14} className={isCloudSyncing ? 'text-indigo-600 animate-pulse' : 'text-green-500'} />}</div><span className="text-sm text-slate-400">{isAdminAuth ? 'Manager View' : 'Toolbox'}</span></div></div>
          <div className="flex-1 overflow-y-auto px-4 md:px-6 pb-2 custom-scrollbar"><nav className="space-y-1">{getOrderedNavItems().filter(item => settings.enabledTools[item.id] !== false).map((item) => (<button key={item.id} onClick={() => { setActiveTool(item.id); setIsSidebarOpen(false); }} className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-200 group ${activeTool === item.id ? 'bg-indigo-600 text-white shadow-md shadow-indigo-100' : 'text-slate-500 hover:bg-slate-100 hover:text-slate-900'}`}><span className={activeTool === item.id ? 'text-white' : 'text-slate-400 group-hover:text-indigo-600'}>{item.id === ToolType.ADMIN ? <Settings size={20} /> : item.icon}</span><div className="flex flex-col items-start text-left min-w-0"><span className="font-semibold text-sm leading-none mb-1 truncate w-full">{item.id}</span><span className={`text-xs truncate w-full ${activeTool === item.id ? 'text-indigo-100' : 'text-slate-400'}`}>{item.description}</span></div>{activeTool === item.id && <ChevronRight size={14} className="ml-auto flex-shrink-0" />}</button>))}</nav></div>
          <div className="p-6 pt-0 space-y-4 flex-shrink-0 bg-white">{isAdminAuth && <button onClick={() => { setIsAdminAuth(false); setActiveTool(ToolType.BASIC_CALC); }} className="w-full flex items-center justify-center gap-2 px-4 py-3 bg-red-50 text-red-600 rounded-xl font-bold text-xs uppercase tracking-widest hover:bg-red-100 transition-all border border-red-100"><LogOut size={16} /> Exit Admin</button>}{(settings.supportEmail || settings.supportPhone) && !isAdminAuth && <div className="border-t pt-4"><div className="bg-slate-50 rounded-xl p-4 space-y-3"><p className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">Support</p>{settings.supportEmail && <div className="flex items-start gap-3"><Mail size={14} className="mt-0.5 text-indigo-500" /><div className="min-w-0"><p className="text-xs font-bold text-slate-700 truncate">{settings.supportEmail}</p></div></div>}{settings.supportPhone && <div className="flex items-start gap-3"><Phone size={14} className="mt-0.5 text-indigo-500" /><div className="min-w-0"><p className="text-xs font-bold text-slate-700 truncate">{settings.supportPhone}</p></div></div>}</div></div>}</div>
        </div>
      </aside>
      ...
      <main className="flex-1 overflow-y-auto p-3 sm:p-4 md:p-6 lg:p-8 transition-all flex flex-col">
        <div className="max-w-[1920px] mx-auto w-full animate-in fade-in slide-in-from-bottom-4 duration-500 flex-1">
          <div className="mb-4 sm:mb-6 ml-2 flex justify-between items-end"><div><h1 className="text-2xl sm:text-3xl md:text-4xl font-black text-slate-800 tracking-tight">{activeTool}</h1><p className="text-slate-500 mt-1 font-medium text-xs sm:text-sm">{navItems.find(i => i.id === activeTool)?.description || (activeTool === ToolType.ADMIN ? 'Manage Global Application Settings' : '')}</p></div>{settings.syncKey && <div className="hidden sm:flex items-center gap-2 px-3 py-1.5 bg-white border border-slate-200 rounded-xl shadow-sm mb-1"><Cloud size={14} className={isCloudSyncing ? 'text-indigo-600 animate-pulse' : 'text-indigo-500'} /><span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Database Sync Active</span></div>}</div>
          <div className="bg-white rounded-[32px] sm:rounded-[40px] shadow-2xl shadow-slate-200/50 p-4 sm:p-6 md:p-10 lg:p-12 border border-slate-100 min-h-[400px]">{renderTool()}</div>
        </div>
