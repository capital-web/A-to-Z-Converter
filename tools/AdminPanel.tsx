...
const AdminPanel: React.FC<AdminPanelProps> = ({ settings, onSave }) => {
  ...
  const handleLinkDevice = async () => {
    ...
    if (remoteData) {
      ...
      alert('Supabase Link Successful! Database synchronized.');
      setManualKey('');
    } else {
    ...
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 pb-6 border-b border-slate-100">
        <div>
          ...
          <p className="text-slate-400 text-sm mt-1">Global Configuration & Supabase Sync</p>
        </div>
      ...
      <div className="flex p-1 bg-slate-100 rounded-2xl w-full md:w-auto self-start overflow-x-auto no-scrollbar">
         <button onClick={() => setActiveTab('settings')} className={`px-6 py-3 rounded-xl text-sm font-bold flex items-center justify-center gap-2 transition-all ${activeTab === 'settings' ? 'bg-white text-indigo-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}><Settings2 size={16} /> General Settings</button>
         <button onClick={() => setActiveTab('cloud')} className={`px-6 py-3 rounded-xl text-sm font-bold flex items-center justify-center gap-2 transition-all ${activeTab === 'cloud' ? 'bg-white text-indigo-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}><Cloud size={16} /> Supabase DB Sync</button>
         <button onClick={() => setActiveTab('hsn')} className={`px-6 py-3 rounded-xl text-sm font-bold flex items-center justify-center gap-2 transition-all ${activeTab === 'hsn' ? 'bg-white text-indigo-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}><Database size={16} /> HSN Directory</button>
      </div>
        ...
        <div className="max-w-4xl space-y-8 animate-in fade-in duration-300">
           <div className="bg-indigo-600 p-8 md:p-10 rounded-[40px] text-white shadow-2xl relative overflow-hidden">
              ...
              <div className="relative z-10 space-y-6">
                 <div><h3 className="text-2xl font-black mb-2">Supabase Cross-Device Cloud</h3><p className="text-indigo-100 font-medium text-sm leading-relaxed max-w-lg">Enable cloud storage to sync your custom HSN codes, application branding, and tool arrangement across all your devices instantly.</p></div>
                 {!localSettings.syncKey ? (<button onClick={handleGenerateKey} className="px-8 py-4 bg-white text-indigo-600 rounded-2xl font-black text-sm uppercase tracking-widest hover:bg-indigo-50 transition-all shadow-xl active:scale-95">Initialize Supabase Database</button>) : (
                   <div className="space-y-4">
                      <div className="space-y-2">
                        ...
                        <div className="flex items-center gap-3">
                           <div className="flex-1 bg-white/10 backdrop-blur-md border border-white/20 rounded-2xl p-4 font-mono font-bold text-lg overflow-hidden truncate">{localSettings.syncKey}</div>
                           <button onClick={copySyncKey} className="p-4 bg-white text-indigo-600 rounded-2xl hover:bg-indigo-50 transition-all shadow-lg active:scale-90">{copiedKey ? <Check size={20} /> : <Copy size={20} />}</button>
                        </div>
                      </div>
           ...
           <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              <div className="bg-white p-8 rounded-[40px] border border-slate-100 shadow-sm space-y-6">
                 ...
                 <div className="space-y-4">
                    ...
                    <button onClick={handleLinkDevice} disabled={isLinking || !manualKey.trim()} className="w-full py-4 bg-slate-900 text-white rounded-2xl font-black text-xs uppercase tracking-widest hover:bg-indigo-600 transition-all disabled:opacity-50">{isLinking ? 'Fetching Data...' : 'Link to Profile'}</button>
                 </div>
              </div>
              <div className="bg-slate-50 p-8 rounded-[40px] border border-slate-200/50 flex flex-col justify-center">
                 <div className="flex items-start gap-4">
                    ...
                    <div><h4 className="font-bold text-slate-700">Encrypted PostgREST Sync</h4><p className="text-xs text-slate-400 mt-1 leading-relaxed">Profile-specific data is isolated by your Sync Key. No personal information is stored outside of your explicit app settings.</p></div>
                 </div>
              </div>
           </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 animate-in fade-in slide-in-from-bottom-2 duration-300">
           <div className="space-y-8">
              <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm space-y-6">
                 <h3 className="text-sm font-bold text-slate-400 uppercase tracking-widest flex items-center gap-2"><LayoutTemplate size={16} /> Branding</h3>
                 <div className="space-y-4">
                    <div className="space-y-2"><label className="text-xs font-bold text-slate-500">App Name</label><input type="text" value={localSettings.appName} onChange={(e) => handleChange('root', 'appName', e.target.value)} className="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl font-bold text-sm" /></div>
                    <div className="space-y-2"><label className="text-xs font-bold text-slate-500">Footer</label><input type="text" value={localSettings.footerText} onChange={(e) => handleChange('root', 'footerText', e.target.value)} className="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl font-bold text-sm" /></div>
                 </div>
              </div>
           </div>
           <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm">
              <h3 className="text-sm font-bold text-slate-400 uppercase tracking-widest flex items-center gap-2 mb-6"><ListOrdered size={16} /> Order</h3>
              ...
              <div className="flex-1 overflow-y-auto space-y-2">
                 {filteredHsnList.slice(0, 50).map(item => (
                    <div key={item.code} className="p-3 bg-slate-50 rounded-xl border border-slate-100 flex items-center justify-between group">
                       ...
                       <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity"><button className="p-1.5 text-slate-400 hover:text-indigo-600"><Edit size={14}/></button><button className="p-1.5 text-slate-400 hover:text-red-500"><Trash2 size={14}/></button></div>
                    </div>
                 ))}
              </div>
           </div>
